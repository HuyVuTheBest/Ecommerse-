						...
						<div class="tm-bg-primary-dark tm-block tm-block-h-auto">
							...
							<div class="row tm-edit-product-row">
								<div class="col-xl-6 col-lg-6 col-md-12">
									...
									<form action="ProductManagerEdit" method="post" enctype="multipart/form-data" class="tm-edit-product-form">
											<div class="form-group mb-3">
													<input type="hidden" name="productId" value ="${i.getProductID()}">
													<label for="name">Tên Sản phẩm</label>
													 <input value="${i.getProName()}" id="name"
														name="ProName" type="text" class="form-control validate"
														required />
												</div>
												<div class="form-group mb-3">
													<label for="description">Mô tả sản phẩm</label>
													<textarea class="form-control validate" name="ProDes" rows="3" required>${i.getDescription()}</textarea>
												</div>
												<div class="form-group mb-3">
												...
												<div class="row">
													<div class="form-group mb-3 col-xs-12 col-sm-6">
														<label for="expire_date">Số
															lượng </label> <input value="${i.getUnitInStock()}"
															id="expire_date" name="ProQuan" type="text"
															class="form-control validate" data-large-mode="true" />
													</div>
													<div class="form-group mb-3 col-xs-12 col-sm-6">
														<label for="stock">Giá Sản phẩm </label> 
															<input value="${i.getUnit_Price()}"
															id="stock" name="ProPrice" type="text"
															class="form-control validate" required />
													</div>
												</div>
												<div class="row">
												<%
											    // Lấy chuỗi kích thước từ biến i.getSizeRange()
											    String sizeRange = "${i.getSizeRange()}";
											
											    // Loại bỏ dấu ngoặc vuông từ chuỗi
											    sizeRange = sizeRange.replace("[", "").replace("]", "").trim();
											%>
													<div class="form-group mb-3 col-xs-12 col-sm-6">
														<label for="expire_date">Size</label> 
														<input value="${i.getSizeRange().toString().replaceAll("[\\[\\]]", "").trim()}"
										...
										<div class="col-xl-6 col-lg-6 col-md-12 mx-auto mb-4">
										    <div class="tm-product-img-dummy mx-auto">
										        <!-- Image display area -->
										        <img id="previewImage" class="img-fluid fixed-size" src="${i.getThumbnail()}" alt="" onclick="document.getElementById('fileInput').click();">
										    </div>
										...
										<div class="col-12">
										    <button type="submit" class="btn btn-primary btn-block text-uppercase">Sửa sản phẩm</button>
										</div>
									</form>
